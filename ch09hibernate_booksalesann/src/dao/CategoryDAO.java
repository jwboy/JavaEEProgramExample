...
public class CategoryDAO extends BaseHibernateDAO {
	...
	public Category findById(java.lang.Integer id) {
		...
		try {
			Category instance = (Category) getSession().get("vo.Category", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Category> findByExample(Category instance) {
		...
		try {
			List<Category> results = (List<Category>) getSession().createCriteria("vo.Category").add(create(instance))
					.list();
	...
	public List<Category> findByNotes(Object notes) {
