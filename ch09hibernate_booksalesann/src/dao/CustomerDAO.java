...
public class CustomerDAO extends BaseHibernateDAO {
	...
	public Customer findById(java.lang.Integer id) {
		...
		try {
			Customer instance = (Customer) getSession().get("vo.Customer", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Customer> findByExample(Customer instance) {
		...
		try {
			List<Customer> results = (List<Customer>) getSession().createCriteria("vo.Customer").add(create(instance))
					.list();
	...
	public List<Customer> findByCustomerAccount(Object customerAccount) {
		return findByProperty(CUSTOMER_ACCOUNT, customerAccount);
	}

	public List<Customer> findByCustomerPassword(Object customerPassword) {
